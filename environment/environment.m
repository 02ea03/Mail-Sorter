...
for i=1:steps
    ...
    theta(1,i) = (1-s(i))*-1 + s(i)*-176*pi/180;                 % Roll angle 0.006
    theta(2,i) = (1-s(i))*0 + s(i)*1*pi/180;            % Pitch angle 0.176
    theta(3,i) = (1-s(i))*0 + s(i)*-0.01*pi/18;            % Yaw angle -0.98
end
...
for i=1:steps
    ...
    x(2,i) = (1-s(i))*0.01 - sin(i*delta/2)-s(i)*1.1; % Points in y
    ...
    theta(3,i) = (1-s(i))*-0.01*pi/18 + s(i)*80.48*pi/18; %yaw angle
end
T = [rpy2r(theta(1,1),theta(2,1),theta(3,1)) x(:,1);zeros(1,3) 1];          % Create transformation of first point and angle
q0 = zeros(1,6);                                                            % Initial guess for joint angles
qMatrix(1,:) = robot.ikcon(T,q0);                                            % Solve joint angles to achieve first waypoint
...
for i=1:steps
    x(1,i) = (1-s(i))*-1.1 + s(i)*1.6; % Points in x
    x(2,i) = (1-s(i))*0 - sin(i*delta/2)+s(i)*1.08; % Points in y
    x(3,i) = (1-s(i))*4.2 + s(i)*4;     % Points in z
    theta(1,i) = (1-s(i))*177*pi/180 + s(i)*-176*pi/180;                 % Roll angle 
    theta(2,i) = (1-s(i))*8*pi/180 + s(i)*1*pi/180;            % Pitch angle
    theta(3,i) = (1-s(i))*47.48*pi/18 + s(i)*-0.01*pi/18; %yaw angle
end
T = [rpy2r(theta(1,1),theta(2,1),theta(3,1)) x(:,1);zeros(1,3) 1];          % Create transformation of first point and angle
q0 = zeros(1,6);                                                            % Initial guess for joint angles
qMatrix(1,:) = robot.ikcon(T,q0);                                            % Solve joint angles to achieve first waypoint
...
%     x(2,i) = (1-s(i))*0 + s(i)*2; % Points in y
%     x(3,i) = (1-s(i))*4 + s(i)*3.4; % Points in z
...
%% Left Blue to Mail
% for i=1:steps
%     x(1,i) = (1-s(i))*-1 + s(i)*1.5; % Points in x
%     x(2,i) = (1-s(i))*2 + s(i)*0; % Points in y
%     x(3,i) = (1-s(i))*3.4 + s(i)*4; % Points in z
%     theta(1,i) = 0;                 % Roll angle 
%     theta(2,i) = 5*pi/9;            % Pitch angle
%     theta(3,i) = 0;                 % Yaw angle
%     %x(i,:) = (1-s(i))*POS1 + s(i)*POS2;
