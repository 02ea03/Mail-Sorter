...
for zOffset = [3.52]
    for yOffset = [0]
        for xOffset = [1.6]          
            paperTri([2]) = trisurf(f,v(:,1) + xOffset,v(:,2) + yOffset, v(:,3) + zOffset ... %delete(paperTri([1]) to delete the
        ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
        end
    end
end
...
for i=1:steps
    ...
    theta(1,i) = (1-s(i))*-1 + s(i)*-176*pi/180;                 % Roll angle
    theta(2,i) = (1-s(i))*0 + s(i)*1*pi/180;            % Pitch angle
